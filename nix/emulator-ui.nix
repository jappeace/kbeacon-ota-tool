...
# Same policy as hatter's emulator harness: up to 10 attempts, with
# retryable-crash.sh aborting immediately on a deterministic native
# failure. The high cap is deliberate: netsim's guest Bluetooth wedges
# on roughly 40% of attempts (the scan finds nothing at all), so a
# generous retry budget is what makes a clean discovery eventually
# land. Each failed attempt cycles the guest stack to recover it.
run_with_retry() {
    local label="$1"; shift
